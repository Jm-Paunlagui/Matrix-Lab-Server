...
def to_view_selected_csv_file(csv_id: int):
    ...
    user_data: User = User.query.with_entities(User.role).filter_by(user_id=user_id).first()
...
def list_csv_file_to_read(csv_id: int, folder_name: str):
    ...
    # Get only the full_name of the user and role to the database.
    user_data: User = User.query.with_entities(User.full_name, User.role).filter_by(user_id=user_id).first()
