...
def error_handler(csv_id: int, error_occurred: str, name_of: str):
    ...
    """
    db.session.add(CsvErrorModel(
        csv_id=csv_id, csv_error=error_occurred, name_of=name_of))
    db.session.commit()
...
def csv_evaluator(file_name: str, sentence_index: int, school_semester: str, school_year: str, csv_question: str):
    ...
    try:
        ...
        error_handler(
            csv_id=csv_id,
            error_occurred="Error in the process of evaluating the csv file with the error: " +
            str(e),
